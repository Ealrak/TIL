...
----
## CSS Style Sheet
* 외부스타일시트 파일 저장은 **styles**폴더에 `파일명CSS` 저장한다.
* 위 파일 생성 후 CSS연결을 원하는 HTML파일 head 위치에 `<link>`태그로 연결한다.
* HTML작성 후 HTMl의 모든 디자인형태를 초기화하는 `reset.css` 반드시 연결해야한다.
* 웹글꼴(Noto Sans KR, Pretendard 등) 연결 시 HTML파일에 `<link>` 태그 연결해야한다.
### head 태그 내에 들어가는 link태그 작성 순서
...
* 실제 디자인데 들어가는 색상은 **rgba 또는 헥사코드**로 입력하고 테스트용으로 입력하는 임시 색상은 영문명으로 입력해야 한다.
...
* `font-weight` - 글자 굵기 | 400 | Regular | `font-weight:400`
* `font-family` - 글꼴 설정 | `font-family:'Pretendard', san-serif;`
* `color` - 글자 색상 | `color:#000; color:rgba(0.0.f.0.4);`
* `background-color` - 배경 색상 | `background-color:#fff;`
* `width` - 가로 크기 | `width:100px;`
* `height` - 세로 크기 | `height:100px;`
* `margin` - 바깥쪽 여백 | `margin-top:20px;`
* `border-radious` - 기울임 | `border-radious:15px;`
## 위치속성 float
* 블록, 인라인 요소를 좌(left), 우(right) 사용하는 위치 속성
* 좌/우 배치하고 싶은 요소가 형제인 경우 사용
* 형제가 3개 이상인 이유 1-> 2 옆으로 두고 3을 내리고 싶은 경우, 형제3에게 `clear:both`를 선연하여 이전 형제의 float 정렬 제거하기
### 오른쪽으로 보내고 싶은 형제 요소가 2개 이상이라면
* 2개를 묶어서 부모로 처리하고 `float:right` 한번만 작성한다.
* float:right는 2번 이상 사용하면 역순이 되므로 **한번만 작성해야 한다.**
* 부모의 높이를 제대로 인식 못하므로 높이를 강제(px)시키거나 `overflow:hidden` 높이를 안주고 영역 재인식을 시켜 float에 의하 오류를 제거해야 한다.
...
* 입력 양식과 선택양식에 따라 같은 스펠링 뜻이 달라지므로 주의해야 한다.
## 양식에 따라 다른 속성의 뜻
...
* input radio, checkbox는 사용자커스텀CSS 디자인이 불가능하기 때문에 사이트에 어울리는 모습으로 디자인하고 싶다면 `display:none` 숨기고 선택글자를 묶은 태그에 `background-image` 디자인 해야 한다.
...
* 속성종류 2. submit : `form action`주소의 `method`값 형태로 전달되는 최종 서버전송버튼
...
* `+, -, *, /` 산술 연산자 모두 사용 가능
* 계산시 **연산자 암 뒤 반드시 공백 포함**해서 작성하기
### CSS Calc() 함수 예시
* `height:calc(50vh - 100px);`
<!-- 사용자가 보는 화면높이의 50%에서 100px을 빼겠다. -->
* `padding:calc(100% - (10px * 3));` 우선순위 괄호 추가 사용 가능
<!-- 10px을 3곱해서 나온 값을 100%에서 빼겠다. -->
* `width:calc(90% - (60px + 5px));` 우선순위도 연산자 앞뒤 공백 필수
* `width:calc(90% - 65px);`
## Flex Layout
* flex는 수평, 수직 1차원 레이아웃으로 메인축과 교차축을 고려하여 다양한 레이아웃을 만들 수 있는 CSS3의 새로운 레이아웃 속성입니다.
* container(부모속성), item(자식속성)에 주는 속성이 다르기 때문에 주의해서 작성해야 한다. **기본 시작은 부모**
* `display:flex` 부모(container)대상에 display 명령어로 해당 레이아웃이 flex라는 선언부터 시작한다.
* (위) flex선언을 진행 시 메인축은 기본값 수평, 교차축은 기본값 수직으로 정렬된다.
## position
### 필수속성 relative, absolute, fixed, sticky
### 선택속성 top, bottom, right, left
* `flex, float`등 포함 위치가 잡혀있는 요소에서 상/하/좌/우로 살짝 이동을 할 때는 `positive:relative;`
* 형제 요소 또는 부모-자식 요소 관계에서 부모 위치를 기준으로 요소를 겹치거나 현재 위치와는 관계없이 멀리 이동할 경우는 `positive:absolute;`
* **주의사항** `absolute` 사용 시 부모들 중 별도의 position속성이 없다면 `body`를 기준으로 위치가 설정되니 부모 중 원하는 기준 대상에 반드시 position 속성을 함께 작성해야 한다. `absolute, relative, fixed, sticky` 모두 가능.(상황에 따라 조합하기)